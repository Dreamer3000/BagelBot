...
you just have to execute $PTree, and your phylogenetic tree will be displayed in the chat.""")
    ...
    @commands.command(aliases = ["RPDB"], help = ("Get information about your protein, if the cif file and ID are supplied. Will not incluse all atoms in the protein to bypass common errors"))
    async def ReadPDB(self, ctx, *args):
        
        try:
            os.chdir(str(ctx.author))
            print(args[0])
            parser = PDBParser(PERMISSIVE = 1) # bypasses common cif encoding errors. Some atoms will be missing as a result
            
            struct_id = str(args[0])
            
            filename = struct_id + ".pdb"
            print("hi1")
            structure = parser.get_structure(struct_id, filename) # problem here for some reason
            print("hi2")
            if len(args) == 1:
                await ctx.send("You haven't given any keywords, so here's some common keywords: ")
                keywords = structure.header["keywords"]
                resolution = structure.header["resolution"]
                await ctx.send(str(keywords))
                await ctx.send(str(resolution))
            os.chdir(master)
        except:
    ...
    async def Blast(self, ctx, *args):
        ...
        try:
            ...
            x = NCBIWWW.qblast("blastn", "nt", Seq(temp[0]))
